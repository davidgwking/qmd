...
function outputResults(results: { file: string; displayPath: string; title: string; body: string; score: number; context?: string | null; chunkPos?: number; firstChunk?: string; hash?: string; docid?: string }[], query: string, opts: OutputOptions): void {
  ...
  if (opts.format === "json") {
    ...
    const output = filtered.map(row => {
      ...
      const intro = (!opts.full && row.firstChunk && row.chunkPos && row.chunkPos > 0) ? row.firstChunk : undefined;
      if (opts.lineNumbers) {
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      // Show intro (first chunk) when snippet is from a different location
      const showIntro = row.firstChunk && row.chunkPos && row.chunkPos > 0;
      if (showIntro) {
        // Truncate intro for display (first ~300 chars)
        let introText = row.firstChunk!;
        if (introText.length > 300) {
          introText = introText.substring(0, 297) + "...";
        }
        console.log(`${c.dim}--- Intro (first chunk) ---${c.reset}`);
        console.log(introText);
        console.log();
        console.log(`${c.dim}--- Best match ---${c.reset}`);
      }
