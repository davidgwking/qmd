 ...
 * Returns the aggregated score, best-scoring chunk position, and first chunk text.
 */
export async function rerankByChunks(
  ...
  db: Database
): Promise<{ file: string; score: number; bestChunkPos: number; firstChunk: string }[]> {
  // 1. Extract all chunks from all candidates
  const allChunks: { file: string; chunkId: string; text: string; pos: number }[] = [];
  const chunksByDoc = new Map<string, { chunkId: string; pos: number; text: string }[]>(); // file -> chunk info
  ...
  for (const doc of candidates) {
    ...
    const docChunks: { chunkId: string; pos: number; text: string }[] = [];

    for (const chunk of chunks) {
      const chunkId = `${doc.file}:${chunk.seq}`;
      allChunks.push({ file: doc.file, chunkId, text: chunk.text, pos: chunk.pos });
      docChunks.push({ chunkId, pos: chunk.pos, text: chunk.text });
    }
    chunksByDoc.set(doc.file, docChunks);
  }
  ...
  // 4. Aggregate scores per document and find best chunk position
  const docScores: { file: string; score: number; bestChunkPos: number; firstChunk: string }[] = [];

  for (const doc of candidates) {
    const docChunks = chunksByDoc.get(doc.file) || [];
    const scores = docChunks.map(c => chunkScores.get(c.chunkId) || 0);
    const aggregated = aggregateChunkScores(scores);

    // Find the best-scoring chunk's position
    let bestChunkPos = 0;
    let bestScore = -1;
    for (let i = 0; i < docChunks.length; i++) {
      ...
      if (score > bestScore) {
        bestScore = score;
        bestChunkPos = docChunks[i]!.pos;
      }
    }

    // Get the first chunk text for intro display
    const firstChunk = docChunks[0]?.text || "";

    docScores.push({ file: doc.file, score: aggregated, bestChunkPos, firstChunk });
  }
